...
- **The admin portal ships four MANDATORY areas.** Dashboard
  (`/admin/dashboard`), Business metrics (`/admin/metrics`), System
  metrics (`/admin/system`) and Feature flags (`/admin/feature-flags`),
  all rendered from **`@forjio/admin-ui`**. Spec:
  `forjio/documentation/2. Technical/13-Admin-Portal-Standard.md`.
  Add your product's own admin surfaces alongside them — pawpado's
  reconcile, plugipay's kyc-inbox — **never instead of them**. What a
  product actually writes is the ADAPTER: fill `MetricsAdapter` in
  `backend/src/lib/business-metrics.ts` against your schema, pass your
  integration probes to `collectSystemHealth()`, and declare flags in
  code with `ensureFeatureFlag()`. The template ships an empty adapter on
  purpose — it still returns a valid, fully shaped payload, so the portal
  renders from day one and each slice can be filled in independently.
  After forking, run `npx prisma migrate dev --name admin_portal_standard`
  to create `feature_flags` + `feature_flag_audits`.
...
- **E2E reaches staging at `http://${{ secrets.STAGING_HOST }}/`** over
  ...
  **`staging-<brand>.forjio.com`** hostname — `e2e-staging` maps it to the
  tailnet IP in the runner's `/etc/hosts` so nginx still routes the
  product's staging vhost by Host header — with `E2E_BYPASS_SECRET` gating
  auth. Staging lives on the shared box (the 2026-06-15 consolidation),
  NOT a per-product droplet. Requires secrets `TS_AUTHKEY` +
  `E2E_BYPASS_SECRET` + `SSH_PRIVATE_KEY` + `STAGING_HOST` +
  `PRODUCTION_HOST`. Register the product on the shared box per the
  spawn-product skill's Phase 1 step 8.
...
- **Admin pages fetch via `adminFetch`** from **`@forjio/admin-ui`** —
  defensive (content-type check before JSON.parse, so HTML error pages
  never surface raw parse errors). This used to be a per-product
  `src/components/admin/ui.tsx`; that copy drifted across the family
  (67 / 165 / 168 lines, absent in six products) and is now centralised.
...
- **shadcn/ui is the family component baseline** (new-york style, slate
  base, CSS-var theming). The template ships `components.json`,
  `src/lib/utils.ts` (`cn()`), and the `src/components/ui/*` primitive
  set (button/card/table/badge/dialog/sheet/alert-dialog/dropdown-menu/
  input/textarea/label/select/checkbox/switch/tabs/accordion/tooltip/
  separator/skeleton/form/sonner + `error-panel`). Build product UI on
  these — don't hand-roll modals (`Dialog`/`Sheet`), confirms
  (`AlertDialog`), or `bg-black/50` overlays, and never use native
  `confirm`/`prompt`/`alert`. Add more components with
  `npx shadcn@latest add <name>` (e.g. `chart` pulls recharts — kept out
  of the baseline as feature-specific). `saas-malapos` is the reference
  implementation. All UI uses the CSS-var tokens
  (`bg-card`/`text-muted-foreground`/`border-border`/`text-primary`/…),
  never raw hex/named colors.
...
- Add `@tanstack/react-query`, state libraries, OR a *second* component
  kit to the template just because plugipay has them. Tailwind +
  `lucide-react` + `@forjio/website-ui` + **shadcn/ui** (radix + cva +
  the `components/ui/*` set) ARE baseline; anything beyond that is a
  per-product choice — keep the template lean. (shadcn became the family
  baseline 2026-06-27 — see the Styling note above; this supersedes the
  prior "no component kits" rule.)
- Add FORJIO4 HMAC middleware from plugipay unless you're actually
  building a payment-style API with signed requests.

### Secrets

- **Secronna (secronna.com) is the Forjio family secrets manager**
  (live since 2026-07-02) — platform-level tokens (GitHub, DO, npm,
  PyPI, …) are readable via its API with a reader token.
- **Spawn-time operational path today**: flat files under
  `/root/.forjio-admin-secrets/` on agent-box (one file per concern:
  `<product>`, `<product>-plugipay`, `<product>-resend`, …), plus box
  env at `/opt/saas/<product>/backend/.env` on the prod droplet and
  the shared staging box. Full migration of per-product secrets into
  Secronna is a separate project — don't block a spawn on it.
- CI secrets are seeded per-repo via `scripts/seed-ci-secrets.sh`
  (`gh secret set`). Never commit `.env`; commit `.env.example`.

### See also

- [`TEMPLATE-UPGRADE-AUDIT.md`](./TEMPLATE-UPGRADE-AUDIT.md) —
  historical (2026-04-20) audit that established the original
  scaffolding; superseded by [`TEMPLATE.md`](./TEMPLATE.md) as of
  2026-07-11.
