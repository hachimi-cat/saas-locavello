 ...
 * "Save changes" → "[Ŝàvé çĥàñĝéŝ~~~]".
 *
 * Run in CI it finds every hardcoded string (unwrapped text stays
 * plain) and every too-tight layout (padding simulates the ~30%
 * expansion of de/fi) BEFORE paying for a single translation. ICU
 * placeholders and quoted literals pass through untouched so the
 * message still formats.
 */
...
export function pseudoize(message: string): string {
  ...
  for (let i = 0; i < message.length; i += 1) {
    ...
    if (depth === 0 && !inQuote) {
      ...
      // Inside an ICU argument: leave the syntax intact, but pseudoize
      // the option bodies' plain text is skipped for simplicity —
      // the outer text carries the visual signal.
      out += ch;
    }
  }
